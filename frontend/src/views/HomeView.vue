    ...
    <header class="sticky top-0 z-30 border-b border-stone-200/80 bg-[#f8fbff]/95 px-5 py-3 backdrop-blur">
      <nav class="mx-auto flex max-w-7xl items-center justify-between gap-4">
        ...
        <div class="hidden items-center gap-6 text-sm font-medium text-stone-600 md:flex">
          ...
          <router-link v-if="showModelPlazaEntry" to="/model-plaza" class="transition-colors hover:text-stone-950">{{ t('nav.modelPlaza') }}</router-link>
        ...
        <div class="flex items-center gap-2">
          <router-link
            v-if="showModelPlazaEntry"
            to="/model-plaza"
            data-testid="mobile-model-plaza"
            class="inline-flex items-center rounded-lg p-2 text-stone-600 hover:text-stone-950 md:hidden"
...
const modelPlazaEnabled = computed(() => resolveFeatureFlag(appStore.cachedPublicSettings, FeatureFlags.modelPlaza))
...
const isHomeContentUrl = computed(() => {
  ...
  return Boolean(homeContentUrl.value) && (content.startsWith('http://') || content.startsWith('https://'))
})
...
async function closeLoginModal(): Promise<void> {
